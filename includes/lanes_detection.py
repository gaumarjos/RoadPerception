...
class LaneDetection():
    ...
    def __init__(self,
        ...
        self.extrinsic_src = extrinsic_src
        self.extrinsic_dst = extrinsic_dst
        ...
        # measured at the base of the camera image
        self.ym_per_pix = 30/720 # meters per pixel in y dimension, lane line is 10 ft = 3.048 meters
        self.xm_per_pix = 3.7/700 # meters per pixel in x dimension, lane width is 12 ft = 3.7 meters
    ...
    def image_processing_pipeline(self, img):
    
        # Make a local copy
        image = np.copy(img)
        
        # Process the image to highlight lines
        # gradient_binary = gradient_x_threshold(image, sobel_kernel=9, thresh=(20, 100))
        color_binary = color_select(image)
        ...
        # Warping
        processed, M, Minv = warp_image(combined_binary,
                                        self.extrinsic_src,
                                        self.extrinsic_dst)
        ...
        return processed, M, Minv
    
    
    def lines_fit_new_frame(self,
                            img,
                            search_area='half',
                            margin=60,
                            minpix = 30,
        ...
        left_linetype, right_linetype = self.detect_linetype(leftx, rightx)
    ...
    def lines_fit_based_on_previous_frame(self,
                                          img,
                                          prev_left_fit,
                                          prev_right_fit,
                                          margin=80,
    ...
    def detect_linetype(self,
                        leftx, 
                        rightx,
                        balance_thresh=1.4,
                        continuous_thresh=8000):
    ...
    def lane_width_validation(self,
                              left_fit,
                              right_fit,
                              h,
                              expected_lane_width=700,
                              expected_lane_width_margin=100):
        lane_width = None
    ...
    def curvature_radius_and_distance_from_centre(self, 
                                                  img,
                                                  left_fit,
                                                  right_fit,
                                                  left_lane_inds,
        ...
        if len(leftx) != 0 and len(rightx) != 0:
            ...
            left_curve_radius = ((1 + (2*left_fit_cr[0]*y_eval*self.ym_per_pix + left_fit_cr[1])**2)**1.5) / np.absolute(2*left_fit_cr[0])
            right_curve_radius = ((1 + (2*right_fit_cr[0]*y_eval*self.ym_per_pix + right_fit_cr[1])**2)**1.5) / np.absolute(2*right_fit_cr[0])
        ...
        if left_fit is not None and right_fit is not None:
            ...
            lane_width = abs(right_fit_x_int - left_fit_x_int) * self.xm_per_pix
            lane_centre_position = (right_fit_x_int + left_fit_x_int) / 2
            centre_dist = (car_position - lane_centre_position) * self.xm_per_pix
            
        return left_curve_radius, right_curve_radius, lane_width, centre_dist


    def draw_overlay(self,
                     original_img, binary_img,
                     left_fit, right_fit,
        ...
        if lane_width is not None:
            ...
            text = 'Lane width: {:04.2f}m ({:04.0f}px)'.format(lane_width, lane_width / self.xm_per_pix)
    ...
    def search_in_image(self, img):
        ...
        lane_width_filter = False
        
        image = img.copy()
        
        # Image processing
        binary, M, Minv = self.image_processing_pipeline(image)
        ...
        if not self.left_line.detected or not self.right_line.detected:
            ...
            right_lane_inds, \
            left_linetype, \
            right_linetype = self.lines_fit_new_frame(binary,
                                                      show=False)
            new_detection = True
        else:
            left_fit, \
            ...
            right_lane_inds, \
            left_linetype, \
            right_linetype = self.lines_fit_based_on_previous_frame(binary,
        ...
        if self.left_line.best_fit is not None and self.right_line.best_fit is not None:
            ...
            right_curve_radius, \
            lane_width, \
            centre_dist = self.curvature_radius_and_distance_from_centre(binary,
            ...
            overlay = self.draw_overlay(image,
                                        binary,
                                        self.left_line.best_fit,
                                        self.right_line.best_fit,
                                        left_lane_inds,
            ...
            if self.video_output:
                if self.video_output_with_camera_background:
                    result_img = cv2.addWeighted(image, 1.0, overlay, 1.0, 0.0)
                else:
                    result_img = overlay
                    
                result = {"img": result_img,
                          "lines": (self.left_line.best_fit, self.right_line.best_fit)}   
            else:
                result = {"img": None,
                          "lines": (self.left_line.best_fit, self.right_line.best_fit)}
            ...
            # Nothing to show
            print("WARNING: No line fit, there's a problem somewhere.")
            if self.full_output:
                return image
            else:
