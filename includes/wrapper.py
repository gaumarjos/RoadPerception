...
class RoadPerception():
    ...
    def process_video(self,
        ...
        for frame in clip.iter_frames():
            ...
            # Process frame
            lane_result = lane_detector.search_in_image(undistorted_frame)
            car_result = car_detector.search_in_image(undistorted_frame)
            
            # Graphically merge the two results
            """
            result = cv2.addWeighted(cv2.addWeighted(undistorted_frame, 1.0, lane_result["img"], 1.0, 0.0),
                                                     1.0, car_result["img"], 1.0, 0.0)
            """                                         
            # Draw bounding boxes
